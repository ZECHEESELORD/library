    ...
    private static MenuFrame frame(String frameId) {
        return new MenuFrame(Component.text(frameId), List.of(new MenuSlot(
                ...
                Map.of(MenuClick.LEFT, MenuInteraction.of(
                        sh.harold.library.menu.ActionVerb.VIEW,
                        new MenuSlotAction.Close())))));
    }

    private static sh.harold.library.menu.MenuButton firstButton() {
        return sh.harold.library.menu.MenuButton.builder(MenuIcon.vanilla("stone"))
                .name("Shared Visual")
                .action(sh.harold.library.menu.ActionVerb.VIEW, "Inspect", context -> { })
                .build();
    }

    private static sh.harold.library.menu.MenuButton secondButton() {
        return sh.harold.library.menu.MenuButton.builder(MenuIcon.vanilla("stone"))
                .name("Shared Visual")
                .action(sh.harold.library.menu.ActionVerb.OPEN, "Inspect", context -> { })
                .build();
    }

    private static sh.harold.library.menu.MenuDisplayItem displayItem(String name) {
        return sh.harold.library.menu.MenuDisplayItem.builder(MenuIcon.vanilla("stone"))
                .name(name)
                .build();
    }
